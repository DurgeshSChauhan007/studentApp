...
const StudentAdmin = () => {
  ...
  async function showStudent(e) {
    ...
    if (sid === '*') {
      try {
        const response = await fetch(`https://studentapp-backend-9ogq.onrender.com/admin/show`);
      ...
      try {
        const response = await fetch(`https://studentapp-backend-9ogq.onrender.com/admin/showByEmail/${sid}`);
        const result = await response.json();
  ...
  const handleDelete = async (email) => {
    try {
      const response = await fetch(`https://studentapp-backend-9ogq.onrender.com/admin/deleteByEmail/${email}`, { method: 'DELETE' });
      const result = await response.json();
      alert(result.msg);
  ...
  const handleUpdate = async (email) => {
    ...
    try {
      const response = await fetch(`https://studentapp-backend-9ogq.onrender.com/admin/updateByEmail/${email}`, {
        method: 'PATCH',
